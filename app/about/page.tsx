...
function Person({ p }: { p: TeamMember }) {
  return <article className="person"><Image src={p.image} width={520} height={520} alt={p.name}/><div><p>{p.role}</p><h3>{p.name}</h3><small><Lines text={p.note} /></small><a href={p.linkedin} target="_blank" rel="noreferrer" aria-label={`${p.name} on LinkedIn`}>LinkedIn <ArrowIcon /></a></div></article>;
}
