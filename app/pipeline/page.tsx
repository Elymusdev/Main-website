...
export default async function Pipeline() {
  ...
  return <PageShell>
    ...
    <section className="dmd-background section-pad"><div className="dmd-copy"><p className="eyebrow">{c.dmdEyebrow}</p><h2>{c.dmdHeading}</h2><div className="dmd-narrative">{c.dmdBody.map((p, i) => <p key={i}>{p}</p>)}</div></div><div className="dmd-figures">{c.figures.map((f) => { const i = f.image ?? { url: f.src, width: f.width, height: f.height }; return <figure key={f.src}><Image src={i.url} width={i.width} height={i.height} alt={f.alt}/><figcaption>{f.caption}</figcaption></figure>; })}</div></section>
