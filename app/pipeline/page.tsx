...
export default async function Pipeline() {
  ...
  return <PageShell>
    ...
    <section className="dmd-background section-pad"><div className="dmd-copy"><p className="eyebrow">{c.dmdEyebrow}</p><h2>{c.dmdHeading}</h2><div className="dmd-narrative">{c.dmdBody.map((p, i) => <p key={i}>{p}</p>)}</div></div><div className="dmd-figures">{c.figures.map((f) => <figure key={f.src}><Image src={f.src} width={f.width} height={f.height} alt={f.alt} unoptimized/><figcaption>{f.caption}</figcaption></figure>)}</div></section>
